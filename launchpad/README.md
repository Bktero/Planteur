...
This is the 'historical' LaunchPad development kit by Texas Instruments.
...
My [first reading](https://mycontraption.com/programming-the-msp430-launchpad-on-ubuntu/) told that is was quite easy to get the needed tool to compile and program the board. You simply need to install a bunch of packages:
...
### Where is msp430-gdb?!
Surprisingly there is no `msp430-gdb` coming with the above package. You need to install a additional package, `msp430-gdb`:
...
### C++
If you want to compile a *.cpp file you should  `msp430-gcc` instead of `msp430-g++`. Indeed the later command automatically adds the option `-lstdc++` to the command line but it seems like there is no C++ library provided with the toolchain. The compilation consequently fails.

Note that the lack of a C++ library also means that standard include files like  `<iostream>` or `<vector>` are not available.
...
As a consequence `-std=c++11` is not supported and it's seems quite fair for such an old toolchain.
...
### Eclipse
cmake -G"Eclipse CDT4 - Unix Makefiles" -DCMAKE_TOOLCHAIN_FILE=msp430.cmake -DCMAKE_ECLIPSE_VERSION=4.5 ~/Documents/GitHub/Planteur/launchpad/

http://stackoverflow.com/questions/37426334/eclipse-freezes-when-creating-a-new-project-or-a-new-class
Eclipse Mars Ubuntu 16.04

GDB Hardware Debugging & Error parsing
http://r6500.blogspot.fr/2014/11/portable-environment-for-msp430.html
...
### Manually
For mspgcc:
Texas Instruments provides gcc toolchains for msp430 and msp432 MCUs : http://www.ti.com/tool/msp430-gcc-opensource

For msp430: http://software-dl.ti.com/msp430/msp430_public_sw/mcu/msp430/MSPGCC/latest/index_FDS.html
On Linux, get download file msp430-gcc-full-linux-installer-4.2.0.36.run
chmod u+x the downloaded executable and run it
A GUI installer appears

Add the bin folder to the PATH

Note that this toolchain version doesn't support #pragma VECTOR=
...
For mspdebug:
https://github.com/dlbeer/mspdebug/releases

If usb.h if not found when calling make, then install libusb:
$ sudo apt install libusb-dev


If readline.h is missing
...
$ udevadm monitor
--> can also be used to get info about usb devices
...
We now have all the information we need to write the udev rule:

$ cd /etc/udev/rules.d/
$ sudo gedit 42-ti-launchpad.rules
[...]
$ cat 42-ti-launchpad.rules 
KERNEL=="ttyACM?", ATTRS{idVendor}=="0451", ATTRS{idProduct}=="f432"
$ sudo /etc/init.d/udev restart

It's no longer necessary to run mspdebug through sudo.

WARNING: rule stopped working when upgrading to Ubuntu 16.04 (from 12.04).
